...
export default function TaskCard({ task, onUpdate }) {
  ...
  const handleDelete = async (id) =>  {
    const supabase = createClient();
    console.log("ID recibido:", task.id);
    ...
    <div className=" w-[30%] border border-gray-600 p-4 rounded-lg bg-gray-800 text-white mb-4  relative ">
      {isEdit ? (
        ...
        <button
            ...
            className="absolute top-3 right-12 p-2 bg-gray-700 hover:bg-gray-600 rounded-full text-gray-300 hover:text-white transition"
            >
            <FaTrashAlt size={16} />
        </button>
        <button
        ...
        >
          ...
          {deleteTask && (
            <div className="absolute top-3 right-5 p-2 bg-red-500 hover:bg-red-600 rounded-full text-white transition">
              <h4>estas seguro de eliminar esta tarea?</h4> 
              <button onClick={()=>handleDelete (task.id)} className="bg-white text-red-500 hover:bg-gray-200 px-4 py-2 rounded font-bold">
                Eliminar
              </button>
              <button onClick={() => setDeleteTask(false)} className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded font-bold">
                Cancelar
              </button>
            </div>
          )}
